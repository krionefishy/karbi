...
Автоматизация не отправляет сообщение — она публикует событие и называет **код бота** и
**аудиторию**. Кому уедет текст, решает модуль уведомлений по подпискам. Поэтому смена
бота или чата не требует правок в автоматизации, а недоступность мессенджера не тормозит
расчёт.
...
| Поле | Что это |
| --- | --- |
| `bot` | код бота, никогда токен |
| `audience` | `seller_subscribers` с `seller_id` или `chat` с конкретным адресом |
| `template` + `params` | из чего собрать текст, а не готовый текст |
| `dedupe_key` | ключ, по которому повтор не превращается во второе сообщение |

Аудитория названа в терминах продюсера: автоматизация знает селлера, а не чаты. Второй
чат на том же селлере поэтому не требует ни строчки изменений на стороне автоматизации.

`template` вместо готового текста — чтобы формулировку можно было поменять без
переотправки событий; в очереди исходящих остаётся и то, из чего текст собран, и он сам.

Неизвестный бот или неизвестный шаблон — баг продюсера, а не сбой доставки: событие
отклоняется с логом, партиция едет дальше.
