...
Принятое решение — не ротация, а **закрепление**: отдельная VPS `wb-egress` с шестью
дополнительными адресами, по одному на селлера, и переезд туда же ключей селлеров и
троттлинга. Целевая картина и контур управления — [WB_EGRESS.md](WB_EGRESS.md).
Пока не реализовано; таблица выше описывает текущее положение.
