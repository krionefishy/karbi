...
Решение — **отдельный сервис-релей на VPS вне РФ** (`relay-nl-1`, Нидерланды). Он
единственный, кто обращается к мессенджеру, и единственный, кто хранит токены ботов.

```mermaid
flowchart LR
    worker["notifications-worker"]
    relay["релей на VPS вне РФ<br/>токены ботов, буфер апдейтов"]
    tg{{"api.telegram.org"}}
...
**Инициатор соединения — всегда основной сервер, в обе стороны.** Из Нидерландов наш
сервер недоступен вообще: ни 443, ни 22, ни ICMP. При этом `ya.ru` и `mail.ru` с той же
машины отвечают, то есть режет не маршрут, а хостинг основного сервера — ровно как
`wildberries.ru` режет иностранные адреса.
