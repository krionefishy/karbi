...
Telegram отвечает `409` на второй `getUpdates` с тем же токеном, и апдейты перестают
приходить обоим поллерам. Ограничение на токен, а не на процесс, поэтому:

- разные боты опрашиваются параллельно внутри одного процесса;
- доставка тоже идёт задачей на бота — иначе один медленный бот тормозил бы рассылку всем
  остальным;
- вторая реплика процесса ломает поллинг, а не масштабирует его.

Список ботов перечитывается из базы примерно раз в минуту, поэтому нового бота
подхватывают без деплоя. Смена токена существующего бота требует перезапуска: токен
расшифровывается один раз при старте поллера.
