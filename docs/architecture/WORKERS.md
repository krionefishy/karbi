...
Одиннадцать долгоживущих процессов. Каждый — цикл: проснуться, посмотреть, что пора, сделать,
...
flowchart TB
    ...
    subgraph core["wb-core-worker"]
        k1["зеркало WB по всем селлерам:<br/>каталог, остатки, отзывы,<br/>задания и поставки FBS, чаты,<br/>остатки по складам WB"]
    ...
    subgraph returns["wb-returns-worker"]
        ...
        rt2["дайджест в 4 слота:<br/>код с QR, адреса ПВЗ"]
        rt3["кнопка «Обновить»"]
        rt4["консьюмер команд бота<br/>/help /returns /qr /extension"]
    end
    subgraph podsort["wb-podsort-worker"]
        ps1["суточные итоги заказов<br/>по 5 суток за проход"]
    end
...
| каждые 10 минут | возвраты WB: отчёт о возвратах за две недели и открытые заявки; архив заявок раз в 6 часов |
| 09:00 / 12:00 / 15:00 / 18:00 | возвраты WB: одно сообщение на кабинет с накопившимся — код дня с QR, готовые и едущие по адресам ПВЗ; без изменений — тишина |
...
| каждые 6 часов | зеркало: остатки по складам WB (отчёт «Остатки на складах») |
| каждый проход | подсорт: недостающие сутки заказов, свежие первыми; неустоявшиеся сутки — не чаще раза в час |
