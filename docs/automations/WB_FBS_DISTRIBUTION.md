...
`DisconnectedSource`, которая отвечает «нечего взять»: без остатка 1С распределение не считается.

Загрузки снимка файлом нет — остаток будет приходить только из обмена. На экране «Остаток 1С» видно
состояние обмена и заготовленные под него таблицы; пока источника нет, экран прямо говорит, что 1С
не подключена.

Снимок принимается целиком или не принимается вовсе. Отвергается, если:

- в нём нет ни одной строки — иначе пустой обмен обнулил бы все пулы;
...
Отклонённые снимки тоже пишутся в журнал.

Пул, пропавший из свежего снимка, обнуляется, а не удаляется: в 1С товара больше нет, значит на WB
его должно стать ноль, но строка держит на себе сопоставление с карточкой.
...
Тело содержит от 1 до 1000 записей и ключуется **баркодом**, а не `chrtId`:

```json
{
  "stocks": [
    {"sku": "2000000000017", "amount": 10}
  ]
}
```

Это не деталь вкуса. WB не валидирует имена полей: запрос с неверным именем вернёт успешные `204`,
не изменив ни одного остатка. Ошибка в этом месте не падает, а тихо перестаёт работать, и заметить
её можно только контрольной вычиткой.

Проверено на боевом кабинете: чтение `POST /api/v3/stocks/{warehouseId}` принимает фильтр `skus`,
учитывает его и возвращает строки с `sku`, `chrtId` и `amount`. Отсутствие строки означает ноль —
позиции без остатка WB не возвращает.
...
«Последний план» определяется по возрастающему номеру, а не по времени создания: двух расчётов в
одну отметку времени достаточно, чтобы порядок стал произвольным, а произвольный порядок здесь
означает публикацию устаревшего плана.
