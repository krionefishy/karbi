...
export function notices(data: Checklist): string[] {
  ...
  if (data.stock_state === "stale") {
    // Оборачиваемость не пишет строк для нулевого остатка, поэтому «всё
    // распродано» и «сбор стоит» отсюда неотличимы — называем оба.
    collected.push(
      "За двое суток нет ни одного товара с остатком: либо всё распродано, либо «Оборачиваемость» не собирает данные.",
    );
  }
