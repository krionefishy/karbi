...
export function notices(data: Checklist): string[] {
  ...
  if (data.stock_state === "stale") {
    collected.push("Товары отбираются по остатку, а сбор остатков этого селлера стоит: данные старше двух суток или сбор не удаётся — таблица пуста.");
  }
  ...
  if (data.reviews_state === "no_snapshot") {
    collected.push("Пункты про отзывы без данных: сбор отзывов этого селлера не удаётся или ни у одной карточки отзывов нет.");
  }
  return collected;
}
