import asyncio
import signal

from backend.infrastructure.logging import configure_logging
from backend.modules.wb_core.infrastructure.wb import EgressGateway
from backend.modules.wb_podsort.infrastructure.wb import WBPodsortStatisticsClient
from backend.shared.settings import Settings, load_settings
from backend.storage.pg import Database
from backend.workers.wb_podsort.worker import PodsortWorker


class PodsortWorkerApplication:
    """Отдельный процесс подсорта: догружает суточные итоги заказов подключённых кабинетов."""

    def __init__(self, settings: Settings | None = None) -> None:
        self.settings = settings or load_settings()
        configure_logging(self.settings.app.log_level)
        self.database = Database()
        self.worker = PodsortWorker(
            self.database, WBPodsortStatisticsClient(EgressGateway(self.settings.egress)), self.settings
        )

    async def run(self) -> None:
        self.settings.validate_runtime_secrets()
        try:
            await self.database.connect(self.settings.database.url, pool_size=2, max_overflow=2)
            await self.worker.run()
        finally:
            await self.database.disconnect()

    def install_signal_handlers(self) -> None:
        loop = asyncio.get_running_loop()
        for name in (signal.SIGINT, signal.SIGTERM):
            loop.add_signal_handler(name, self.worker.stop)


async def run_worker() -> None:
    application = PodsortWorkerApplication()
    application.install_signal_handlers()
    await application.run()
